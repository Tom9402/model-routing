...
子模型调用失败时自动按 Luna → Terra → Sol → 当前主代理的顺序接力，从原选模型向后尝试，
跳过不可用或已失败的模型，不逐次询问。切换遵守现有预算与权限；共同环境故障和审批拒绝
先处理原因，不靠切换模型绕过。完整规则见 `assets/AGENTS.md`。

## 一句话安装（推荐）

把下面这句话发给目标设备上的 Codex：

> 请根据 https://github.com/Tom9402/model-routing/blob/main/SKILL.md， 安装并启用 model-routing。
